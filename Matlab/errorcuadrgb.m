function [m,n,r,dm,dn]=errorcuadrgb(x,y,ddx,ddy,RGB)
...
for rw=1:rows
  ...
  if length(x)==length(y)
    ...
    %% Representación
    if RGB == 'm'
        hold on; plot(x,f,Color=rgb(m),LineWidth=2);
    elseif RGB == 'n'
        hold on; plot(x,f,Color=rgb(n),LineWidth=2);
    else
        nRGB = size(RGB); nRGB = nRGB(1); rgbw = rw;
          if rem(rows,nRGB)==0; rgbw=rem(rw,nRGB); if rgbw==0; rgbw=nRGB; end; end
        hold on; plot(x,f,Color=rgb(RGB(rgbw,:)),LineWidth=0.5);
    end
